...
namespace SmartCertification.Infrastructure
{
    public class ExamRepository : IExamRepository
    {
        ...
        //        {
        //            throw new Exception($"UserId {exam.UserId} does not exist");
        //        }
        ...
        {
            return _context.ExamQuestions.FirstOrDefaultAsync(eq => eq.ExamId == examId && eq.ExamQuestionId == examQuestionId);
        }

        public async Task<List<ExamQuestion>> GetExamQuestionsAsync(int examId)
        {
            return await _context.ExamQuestions.Where(eq=>eq.ExamId == examId).ToListAsync();
        }
        ...
        public async Task<List<Question>> GetRandomQuestionsAsync(int courseId, int count)
        {
            ...
            mainCourses.ForEach(e =>
            {
                ...
                {
                    courseIds = _context.Courses.Where(w => w.Title.ToLower().StartsWith(courses.Title.ToLower())).Select(s => s.CourseId).ToList();
        ...
        {
            var result = await _context.Exams.Join(_context.Courses,
                exam => exam.CourseId,
        ...
        public async Task SaveExamStatusAsync(int examId, string feedback)
        {
            ...
            if(existingExam != null)
            {
                ...
                existingExam.FinishedOn = DateTime.Now; 
        ...
        {
            var examQuestions = await _context.ExamQuestions.Where(eq => eq.ExamId == examId).ToListAsync();
            var questionIds = examQuestions.Select(eq => eq.QuestionId).Distinct();
            var correctChoices = await _context.Choices.Where(c => questionIds.Contains(c.QuestionId) &&
            c.IsCorrect).ToListAsync();

            var correctChoiceMap = correctChoices.GroupBy(c => c.QuestionId).ToDictionary(
                g => g.Key,
